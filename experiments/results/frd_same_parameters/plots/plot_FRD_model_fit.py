...
TEXTS = {
    True: {  
        ...
        "title": 'Fréchet Radar Distanz vs. Datensatzgröße',
        "legend_prefix": 'Projektion-Dim',
        "print_const": "konst",
        "print_slope": "steigung",
    },
    False: {  
        "xlabel": r'Sample size $N$',
        "ylabel": r'Fréchet Radar Distance',
        "title": 'Fréchet Radar Distance vs Dataset Size by Feature Dimension',
...
if __name__ == "__main__":
    ...
    for dim in unique_features:
        ...
        y = [fdist  for fd, fdist in zip(df["feature_dim"], df["frechet_distance"]) if fd == dim] 
        weights = np.sqrt(np.array(x))

        ax.scatter(x, y, marker='o', label=f'{T["legend_prefix"]}={dim}')

        X_with_intercept = sm.add_constant(1 / np.array(x))
    ...
    fig.savefig(path_plots / "FRD_model_plot.png", dpi=300)